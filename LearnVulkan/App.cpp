...
void App::cleanup()
{
    ...
    vkDestroyPipelineLayout(device, pipelineLayout, nullptr);   //The pipeline layout will be referenced throughout the program's lifetime, so it should be destroyed at the end
...
void App::createInstance()
{
    ...
    if (enableValidationLayers && !checkValidationLayerSupport()) {        //check if all of the layers in validationLayers exist in the availableLayers list.
        throw std::runtime_error("validation layers requested, but not available!");
    }
}

void App::checkExtension()
{
    uint32_t extensionCount = 0;
    vkEnumerateInstanceExtensionProperties(nullptr, &extensionCount, nullptr);  //get num of available extebsion
    std::vector<VkExtensionProperties> extensions(extensionCount);
    vkEnumerateInstanceExtensionProperties(nullptr, &extensionCount, extensions.data());   //get details
...
{
    //start with the basic surface capabilities
...
{
    //It's a lot of work to set all of this up from scratch, 
    //but the advantage is that we're now nearly fully aware of everything that is going on in the graphics pipeline! 
    //This reduces the chance of running into unexpected behavior 
    //because the default state of certain components is not what you expect.
    ...
    // assign vert shader to a specific pipeline stage
    VkPipelineShaderStageCreateInfo vertShaderStageInfo{};
    vertShaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    vertShaderStageInfo.stage = VK_SHADER_STAGE_VERTEX_BIT;
    vertShaderStageInfo.module = vertShaderModule;
    vertShaderStageInfo.pName = "main";
    // assign frag shader to a specific pipeline stage
    ...
    // defining an array that contains these two structs
    ...
    //limited amount of the state that we've specified in the previous structs can actually be changed without recreating the pipeline
    ...
    //fully define the functionality of the graphics pipeline,
    ...
    // reference to the render pass and the index of the sub pass where this graphics pipeline
    pipelineInfo.renderPass = renderPass;
    pipelineInfo.subpass = 0;
    //pipeline derivatives
    pipelineInfo.basePipelineHandle = VK_NULL_HANDLE; // Optional
    pipelineInfo.basePipelineIndex = -1; // Optional
    // inally create the graphics pipeline 
...
void App::drawFrame()
{
    ...
    VkPipelineStageFlags waitStages[] = { VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT };//implementation can already start executing our vertex shader 
...
std::vector<char> App::readFile(const std::string& filename)    //read shader bytecode binary
{
    ...
    //back to the beginning of the file and read all of the bytes at once:
    file.seekg(0);
