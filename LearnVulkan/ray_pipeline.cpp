...
void VkRayTracingApplication::initVulkan(Scene* scene)
{
    ...
    //createLogicalConnection_ray_query(this);
    createSwapchain(this);
    //createSwapchain_ray_query(this);
    ...
    //createGraphicsPipeline(this);  //ray query
    createCommandBuffers(this);
    //createCommandBuffers_ray_query(this,scene);
...
void VkRayTracingApplication::mainLoop(VkRayTracingApplication* app, Camera* camera)
{
    ...
    while (!glfwWindowShouldClose(app->window)) {
        ...
        //drawFrame_ray_query(app, camera);
    }
...
void VkRayTracingApplication::createLogicalConnection_ray_query(VkRayTracingApplication* app) {
    app->graphicsQueueIndex = -1;
    app->presentQueueIndex = -1;
    app->computeQueueIndex = -1;
...
void VkRayTracingApplication::createCommandBuffers(VkRayTracingApplication* app)
{
    ...
    for (int x = 0; x < app->imageCount; x++) {
        ...
        VkDeviceAddress shaderBindingTableBufferDeviceAddress = pvkGetBufferDeviceAddressKHR(app->logicalDevice, &shaderBindingTableBufferDeviceAddressInfo);//Acess violation reading location
...
void VkRayTracingApplication::createFramebuffers(VkRayTracingApplication* app) {
    void* returnPt= malloc(sizeof(VkFramebuffer) * app->imageCount);
    if (returnPt != NULL) {
        app->swapchainFramebuffers = (VkFramebuffer*)returnPt;
    }
    else {
        cout << "malloc failed\n";
    }
    ...
    for (int x = 0; x < app->imageCount; x++) {
        ...
        VkResult errorCode = vkCreateFramebuffer(app->logicalDevice, &framebufferCreateInfo, nullptr, &app->swapchainFramebuffers[x]);//Acess violation reading
...
void VkRayTracingApplication::createSwapchain_ray_query(VkRayTracingApplication* app) {
    VkSurfaceCapabilitiesKHR surfaceCapabilities;
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(app->physicalDevice, app->surface, &surfaceCapabilities);
...
void VkRayTracingApplication::createCommandBuffers_ray_query(VkRayTracingApplication* app, Scene* scene) {
    app->commandBuffers = (VkCommandBuffer*)malloc(sizeof(VkCommandBuffer) * app->imageCount);
    ...
    for (int x = 0; x < app->imageCount; x++) {
        ...
        renderPassBeginInfo.framebuffer = app->swapchainFramebuffers[x];        //Access violation reading
...
void VkRayTracingApplication::drawFrame_ray_query(VkRayTracingApplication* app, Camera* camera) {
    vkWaitForFences(app->logicalDevice, 1, &app->inFlightFences[app->currentFrame], VK_TRUE, UINT64_MAX);
